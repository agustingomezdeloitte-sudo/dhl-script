...
DHL_USER = os.getenv("USER_DHL", "").strip()
DHL_PASS = os.getenv("PASS_DHL", "").strip()

CHILE_TZ = os.getenv("CHILE_TZ", "America/Santiago")
TZ_CHILE = ZoneInfo(CHILE_TZ)

BASE_DIR = Path(os.getenv("DHL_BASE_DIR", "./runtime"))
LOG_DIR = Path(os.getenv("DHL_LOG_DIR", str(BASE_DIR / "logs")))
SCREENSHOT_DIR = Path(os.getenv("DHL_SCREENSHOT_DIR", str(BASE_DIR / "screenshots")))
STATUS_FILE = Path(os.getenv("DHL_STATUS_FILE", str(LOG_DIR / "status_dhl_bot.txt")))
PROFILE_DIR = Path(os.getenv("DHL_PROFILE_DIR", str(BASE_DIR / "playwright_profile")))

LOG_DIR.mkdir(parents=True, exist_ok=True)
SCREENSHOT_DIR.mkdir(parents=True, exist_ok=True)
PROFILE_DIR.mkdir(parents=True, exist_ok=True)

# Si quieres cambiar correos sin tocar código, crea secret/variable:
# DHL_EXPORT_EMAILS=correo1@dominio.com,correo2@dominio.com
CORREOS_EXPORT_ENV = os.getenv("DHL_EXPORT_EMAILS", "").strip()
if CORREOS_EXPORT_ENV:
    CORREOS_EXPORT = [x.strip() for x in CORREOS_EXPORT_ENV.split(",") if x.strip()]
else:
    CORREOS_EXPORT = [
...
def main():
    if not DHL_USER or not DHL_PASS:
        raise RuntimeError("Faltan credenciales USER_DHL / PASS_DHL en Secrets de GitHub")
